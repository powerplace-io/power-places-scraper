...
def get_address(place):
    try:
        return "{street} {number} {zip_code} {city}".format(
            street=place['tags']['addr:street'],
            number=place['tags']['addr:housenumber'],
            zip_code=place['tags']['addr:postcode'],
            city=place['tags']['addr:city'],
        )
    except KeyError:
        return place['tags']['addr:full']
...
def get_google_info(place, acceptable_distance = 100):
    """
    ...
    place_identifier = "{name} {address}".format(
    ...
    if any_info:
        if lat is None or lng is None:
            debugging_msg = "no_coords"
        else:
            distance = geo_distance((place['lat'], place['lng']), (lat, lng)).meters

            if (distance > acceptable_distance):
                debugging_msg = "nok_distance"
            else:
                debugging_msg = "ok"
            result['position'] = {'lat': lat, 'lng': lng}
    else:
        debugging_msg = "no_info"

    # Add debuggin info
    debugging_info = dict(
        distance_between_places=distance,
        status=debugging_msg,
        any_info=any_info,
    ...
    result['debug_info'] = debugging_info
